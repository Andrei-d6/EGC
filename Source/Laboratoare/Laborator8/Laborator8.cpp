...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.37, 0.72, 0.89));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1, 0.13, 0.53));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1, 0.5, 1));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.213, 0.45, 0.723));
	}
	...
	// Render the point light in the scene
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.17, 0.43, 0.83));
	}
}
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		lightDirection = glm::normalize(rotationMatrix * glm::vec4(lightDirection, 0));
